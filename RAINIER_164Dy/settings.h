...
const int g_nEvent = 2.e6; // number of events per realization (and ExI in bExSpread)
...
#ifdef bLD_Table
#include "nld.dat" // made for Gilbert and Cameron 56Fe, not 144Nd
...
#define bGSF_Table
#ifdef bGSF_Table
#include "gsf.dat" // just placeholder values in this file for now
#else
...
const double g_dExIMax = 8.; // MeV; above max population energy
const char popFile[] = "pop.dat"; // made from TALYS "outpopulation y"
// make sure to match # of discrete bins. See ReadPopFile() bins + maxlevelstar + 1 = g_nExPopI
const int g_nExPopI = 90; // bins 0-89; bins + maxlevelstar + 1 = g_nExPopI
const int g_nSpPopIBin = 30; // spins 0-29
const double g_dExRes = 0.15 / 2.355; // excitation resolution on g_ah2ExEg
